...
\section{WLS Relaxation: A simple linear time relaxation}

In this section we propose a novel convex relaxation for the Truncated Least Squares problem that can be computed in linear time and can be minimized by solving a simple weighted least squares problem.
...
The idea of the WLS-relaxation is the observation that this function is only non-convex because of the truncation -- the residuals itself are convex. Since sums of convex functions are convex, we can focus on relaxing each term separately.

For this, we first find the minimum and maximum values of each residual $r_i$ is over a given interval, denoted as $r_{min}^i$ and $r_{max}^i$ respectively. We then re-weight these residuals such that their maximum becomes $\epsilon^2$ and thus they are not truncated over the entire interval (Fig. \ref{fig:wlsrelaxideawithrelax}).
...
In the following we show how $r_{min}$ and $r_{max}$ can easily be computed analytically for different models (i.e. translation and rotation), given spherical intervals (balls).
